...
export async function POST(req: Request) {
  try {
    ...
    if (!unitId || typeof unitId !== 'string') {
      return NextResponse.json({ error: 'Missing unitId', code: 'BAD_BODY' }, { status: 400 });
    }
    ...
    if (progressError || eventsError) {
      console.error('Progress/events query error:', progressError || eventsError);
      return NextResponse.json({ error: 'Failed to check progress', code: 'DATABASE_ERROR' }, { status: 500 });
    }

    const progressTimeMs = progress?.time_ms || 0;
    const recentTimeMs = (recentEvents || []).reduce((sum, event) => sum + event.ms_delta, 0);
    ...
    if (attemptError || !attempt) {
      ...
      return NextResponse.json({ error: 'Failed to create attempt', code: 'DATABASE_ERROR' }, { status: 500 });
    }
    ...
    if (questionsError || !questions || questions.length === 0) {
      ...
      return NextResponse.json({ error: 'No questions available', code: 'NO_QUESTIONS' }, { status: 500 });
    }

    // Insert attempt_items stubs
    ...
    if (itemsError) {
      ...
      return NextResponse.json({ error: 'Failed to create quiz items', code: 'DATABASE_ERROR' }, { status: 500 });
    }
