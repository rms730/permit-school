...
for (const k of REQUIRED) if (!process.env[k]) { console.error('[ingest] Missing env var:', k); process.exit(1); }

const argv = process.argv.slice(2);
const args = Object.fromEntries(
  argv.map((a,i)=>a.startsWith('--')
    ? [a.replace(/^--/,'').split('=')[0], (a.includes('=')?a.split('=').slice(1).join('='):(argv[i+1]&&!argv[i+1].startsWith('--')?argv[i+1]:true))]
    : []).filter(Boolean)
);
const url = args.url;
...
if (!url) { console.error('Usage: node tools/ingest/ingest_handbook.mjs --url <PDF_URL> --jurisdiction CA --lang en --source <SOURCE_URL>'); process.exit(1); }
...
function fetchPdfBuffer(pdfUrl) {
  return new Promise((resolve, reject) => {
    https.get(pdfUrl, (res) => {
      if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) return fetchPdfBuffer(res.headers.location).then(resolve).catch(reject);
      if (res.statusCode !== 200) return reject(new Error('HTTP '+res.statusCode));
      const data = []; res.on('data', d=>data.push(d)); res.on('end', ()=>resolve(Buffer.concat(data)));
    }).on('error', reject);
  });
}

function chunkText(full, target=1400, overlap=200) {
  const clean = full.replace(/\r/g,' ').replace(/[ \t]+/g,' ').replace(/\n{2,}/g,'\n');
...
async function main() {
  ...
  const { data: jur, error: jurErr } = await sb.from('jurisdictions').select('id').eq('code', jurisdictionCode).single();
  ...
  for (let i = 0; i < chunks.length; i += batchSize) {
    ...
    if (error) { console.error('[ingest] insert error', error); process.exit(1); }
