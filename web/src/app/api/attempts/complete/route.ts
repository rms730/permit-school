...
export async function POST(req: Request) {
  try {
    ...
    if (itemsError || !items || items.length === 0) {
      ...
      return NextResponse.json({ error: 'No quiz items found', code: 'NOT_FOUND' }, { status: 404 });
    }
    ...
    if (updateError) {
      ...
      return NextResponse.json({ error: 'Failed to complete attempt', code: 'DATABASE_ERROR' }, { status: 500 });
    }

    // Update skill mastery via simple EWMA (α = 0.3)
    const alpha = 0.3;
    const skillResults = items.reduce((acc, item) => {
      if (!acc[item.skill]) {
