...
export async function POST(req: Request) {
  try {
    ...
    if (!attemptId || typeof attemptId !== 'string') {
      return NextResponse.json({ error: 'Missing attemptId', code: 'BAD_BODY' }, { status: 400 });
    }

    if (!itemNo || typeof itemNo !== 'number' || itemNo < 1) {
      return NextResponse.json({ error: 'Invalid itemNo', code: 'BAD_BODY' }, { status: 400 });
    }

    if (!answer || typeof answer !== 'string') {
      return NextResponse.json({ error: 'Missing answer', code: 'BAD_BODY' }, { status: 400 });
    }
    ...
    if (attempt.student_id !== user.id) {
      return NextResponse.json({ error: 'Not your attempt', code: 'FORBIDDEN' }, { status: 403 });
    }
    ...
    if (updateError) {
      ...
      return NextResponse.json({ error: 'Failed to save answer', code: 'DATABASE_ERROR' }, { status: 500 });
    }
