...
export async function POST(req: Request) {
  try {
    ...
    if (!msDelta || typeof msDelta !== 'number' || msDelta < 1000 || msDelta > 300000) {
      return NextResponse.json({ error: 'Invalid msDelta', code: 'BAD_BODY' }, { status: 400 });
    }
    ...
    if (insertError) {
      ...
      return NextResponse.json({ error: 'Failed to record seat time', code: 'DATABASE_ERROR' }, { status: 500 });
    }
    ...
    const newTimeMs = Math.min(currentTimeMs + msDelta, unit.minutes_required * 60000);
    ...
    if (upsertError) {
      ...
      return NextResponse.json({ error: 'Failed to update progress', code: 'DATABASE_ERROR' }, { status: 500 });
    }
