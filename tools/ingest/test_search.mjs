...
const query =
  ...
  "right-of-way at four-way stop in California";

const SUPABASE_URL = process.env.SUPABASE_URL;
const SUPABASE_KEY =
  process.env.SUPABASE_SERVICE_ROLE_KEY ||
  process.env.SUPABASE_ANON_KEY || // publishable key in new Supabase UI
...
async function main() {
  ...
  for (const r of data) {
    console.log(
      `- id=${r.id} dist=${r.distance.toFixed(4)} src=${r.source_url || ""}`,
    );
